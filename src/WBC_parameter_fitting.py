...
# 创建用于存储图像的文件夹
output_dir = "Results/Figures_WBC"
...
def analyze_patient(patient_id, data1, data2):
    ...
    # 提取我们关心的变量（例如 xma）
    xma_values_at_timeWBC_opt = np.interp(timeWBC, solution_opt.t, solution_opt.y[-1])

    # Calculate R^2
    y_obs = xma_data  # Observed data
    y_pred = xma_values_at_timeWBC_opt  # Predicted data
    ...
    time_ge_0_data = time_wbc_data[time_wbc_data['time'] >= 0]

    # 提取 time 和 WBC 列的值
    time_ge_0 = time_ge_0_data['time'].values
    wbc_ge_0 = time_ge_0_data['WBC'].values
    ...
    plt.scatter(time_ge_0, wbc_ge_0, label='Experimental data')
    plt.xlabel('Time (days)')
    plt.ylabel('WBC count ($10^9/L$)')
...
def main():
    ...
    # 将结果写入CSV文件
    with open('optimized_parameters_and_R2_WBC.csv', 'w', newline='', encoding='utf-8') as csvfile:
