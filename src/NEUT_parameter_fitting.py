...
# 创建用于存储图像的文件夹
output_dir = "Results/Figures_NEUT"
...
def analyze_patient(patient_id, data1, data2):
    ...
    def objective(p):
        ...
        # # 计算相对误差
        # relative_error = np.sum(((xma_values_at_timeNEUT - xma_data) / np.maximum(np.abs(xma_data), 1e-8)) ** 2)
        # # 计算归一化的均方误差（NMSE）
    ...
    # 参数边界
    lb = [1e-3, 0.1, 0.1, 1e-3, 1e-3]  # 每个参数的下界   B, gamma, ktr, slopeA, slopeD = p
    ub = [15, 1, 1.5, 15, 15]
    ...
    # 提取我们关心的变量（例如 xma）
    xma_values_at_timeNEUT_opt = np.interp(timeNEUT, solution_opt.t, solution_opt.y[-1])

    # Calculate R^2
    y_obs = xma_data  # Observed data
    y_pred = xma_values_at_timeNEUT_opt  # Predicted data
    ...
    time_ge_0_data = time_NEUT_data[time_NEUT_data['time'] >= 0]

    # 提取 time 和 NEUT 列的值
    time_ge_0 = time_ge_0_data['time'].values
    NEUT_ge_0 = time_ge_0_data['NEUT'].values
    ...
    plt.scatter(time_ge_0, NEUT_ge_0, label='Experimental data')
    plt.xlabel('Time (days)')
    plt.ylabel('NEUT count ($10^9/L$)')
...
def main():
    ...
    # 将结果写入CSV文件
    with open('optimized_parameters_and_R2_NEUT.csv', 'w', newline='', encoding='utf-8') as csvfile:
